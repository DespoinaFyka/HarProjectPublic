...
from utils.data_processing import estimate_standardization_params, project_features_to_selected_sensors, standardize_features
from utils.labels_utils import get_label_pretty_name
from sklearn.metrics import f1_score, recall_score, precision_score, confusion_matrix

def train_logistic_regression_model(X_train,Y_train,M_train,feat_sensor_names,label_names,sensors_to_use,target_label):
    ...
    # There may be missing sensor-features (represented in the data as NaN).
    # Imputing a value of zero (equivalent to assuming average value).
    # Other solution would be to also further select examples - only those that have values for all the features.
    ...
    # To avoid a trivial classifier important to counter-balance the pos/neg classes:
...
def test_logistic_regression_model(X_test,Y_test,M_test,timestamps,feat_sensor_names,label_names,model):
    ...
    # Precision:
    # May be too sensitive to rare labels.
    ...
    'y_timestamps': timestamps.tolist()
}